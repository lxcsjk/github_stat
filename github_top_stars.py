...
while start <= limit:
    ...
    url = 'https://api.github.com/search/repositories?q=stars:%d..%d&sort=stars&per_page=100&page=%d&order=asc' % (
        start, end, page)
    print(url)
    ...
    if total_count > 0:
        ...
        for repo in repos:
            ...
            target.write("%d-%d\t%s\t%s\t%d\t%d\t%s\t%d\t%d" % (
                start, end, name, owner, stars, watchers, language, forks_count, issues_count))
            target.write("\n")
    start = end + 1
    if start >= 3200:
        step = 100
